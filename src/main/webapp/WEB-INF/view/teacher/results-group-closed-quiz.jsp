...
<div class="container">
    ...
    <table class="table">
        <tr>
            ...
            <th style="width: 25%">Passed</th>
        </tr>
        <c:forEach items="${students}" var="student" varStatus="status">
            <c:set var="i" value="${status.index}"/>
            <tr>
                <td><a href="/teacher/students/${student.userId}">${student.lastName} ${student.firstName}</a></td>
                <td>${closedResults[i].result} / ${closedResults[i].score}</td>
                <td>${closedResults[i].attempt}</td>
                <td><duration:format value="${closedResults[i].timeSpent}"/></td>
                <td><localDateTime:format value="${closedResults[i].finishDate}"/></td>
            </tr>
        </c:forEach>
    </table>
